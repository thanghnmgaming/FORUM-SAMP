!function($, window, document, _undefined)
{
	...
	XF.Lightbox = XF.Element.newHandler({
		...
		initPlugins: function()
		{
			...
			/**
			 * Plugin for LightGallery to display an icon which allows an image to be liked.
			 * Add data-lb-like-ctrl="selector to corresponding element with data-xf-click=like"
			 * to a.js-lbImage
			 */
			var likerDefaults = {
				liker: true
			...
			var Liker = function(element)
			{
				// You can access all lightgallery variables and functions like this.
				this.core = $(element).data('lightGallery');

				this.$el = $(element);
				this.core.s = $.extend({}, likerDefaults, this.core.s);

				if (this.$el.data('lb-liker'))
				{
					this.init();
					return this;
				}
			...
			Liker.prototype.init = function()
			{
				if (this.core.s.liker)
				{
					var $outer = this.core.$outer,
						$liker = $('<span id="lg-liker" class="lg-icon"><i></i><span class="label"></span></span>'),
						$toolbar = $outer.find('.lg-toolbar'),
						$position = $toolbar.find('#lg-download'),
						running = false,
						_core = this.core;

					this.$el.on('onSlideItemLoad.lg onAfterSlide.lg', function(e, index)
					{
						if (running) return;
						running = true;

						var $a = $($(this).find('.js-lbImage').get(index)),
							$likeCtrl = XF.findRelativeIf($a.data('lb-liker-ctrl'), $a);

						_core.$likeCtrl = $likeCtrl;

						if ($likeCtrl.length)
						{
							$likeCtrl
								.off('xf-click:after-handleAjax.XFLikeClick')
								.on('xf-click:after-handleAjax.XFLikeClick', function(e, LikeClick, data)
							{
								$liker.toggleClass('is-liked', $likeCtrl.hasClass('is-liked'));
							});

							$liker
								.toggleClass('is-liked', $likeCtrl.hasClass('is-liked'))
								.off('click')
								.on('click', function(e)
							{
								$likeCtrl.trigger('click', e);
							});
						}
					...
					if ($position.length)
					{
						$liker.insertBefore($position);
					}
					else
					{
						$toolbar.append($liker);
					}
				}
			};
			 ...
			 * before destroying the gallery
			 */
			Liker.prototype.destroy = function()
			{
				this.$el.off('onSlideItemLoad.lg onAfterSlide.lg');

				if (this.core.$likeCtrl)
				{
					this.core.$likeCtrl.off('xf-click:after-handleAjax.XFLikeClick');
				}
			};

			$.fn.lightGallery.modules.newwindow = NewWindow;
			$.fn.lightGallery.modules.captionloader = CaptionLoader;
			$.fn.lightGallery.modules.liker = Liker;
		}
	});
