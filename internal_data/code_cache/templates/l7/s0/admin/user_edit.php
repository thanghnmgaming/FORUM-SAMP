	...
	if ($__templater->method($__vars['user'], 'isUpdate', array())) {
		...
		if (!$__vars['user']['is_super_admin']) {
			$__compilerTemp4 .= '
					...
					<a href="' . $__templater->func('link', array('users/manage-watched-threads', $__vars['user'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Quản lý các chủ để quan tâm' . '</a>
				';
		}
		$__compilerTemp5 = '';
...
<div class="block">
	...
	';
		...
		if ($__vars['user']['is_moderator']) {
			$__compilerTemp6 .= '
					<li><a href="' . $__templater->func('link', array('moderators', ), true) . '">' . 'Quản trị' . '</a></li>
				';
		}
		$__compilerTemp6 .= '
				';
	...
	if ($__vars['user']['user_id']) {
		...
		$__compilerTemp10 .= $__templater->formRow('
			...
			'data-xf-click' => 'overlay',
							...
							<a href="' . $__templater->func('link', array('users/avatar', $__vars['user'], ), true) . '" data-xf-click="overlay">' . 'Sửa ảnh đại diện' . '</a>
						', array(
			'label' => 'Ảnh đại diện',
		)) . '
						' . $__templater->formRow('
	...
	if ($__vars['user']['user_id']) {
		$__compilerTemp12 .= '
							';
		if (!$__vars['user']['is_moderator']) {
			$__compilerTemp12 .= '<a href="' . $__templater->func('link', array('moderators', ), true) . '">' . 'Đặt thành viên này làm kiểm duyệt viên' . '</a>';
		}
		$__compilerTemp12 .= '
							';
		...
		$__compilerTemp12 .= '
							';
		if (!$__vars['user']['is_admin']) {
			$__compilerTemp12 .= '<a href="' . $__templater->func('link', array('admins', ), true) . '">' . 'Đặt thành viên này làm quản trị viên' . '</a>';
		}
		$__compilerTemp12 .= '
						';
	}
