...
';
	...
	if (!$__templater->test($__vars['availableFields'], 'empty', array())) {
		...
		$__compilerTemp1 .= '
				...
				' . $__templater->formRow('
					' . $__templater->filter('Không có', array(array('parens', array()),), true) . ' <a href="' . $__templater->func('link', array('custom-thread-fields/add', ), true) . '" target="_blank">' . 'Thêm trường' . '</a>
				', array(
			'label' => 'Available fields',
		)) . '
			';
