...
'addon_list_item' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['addOn']['is_processing']) {
		$__finalCompiled .= '
						<span class="label label--error" data-xf-init="tooltip" title="' . $__templater->filter('Last action' . $__vars['xf']['language']['label_separator'], array(array('for_attr', array()),), true) . ' ' . ($__templater->escape($__vars['addOn']['last_pending_action']) ?: $__templater->filter('N/A', array(array('for_attr', array()),), true)) . '">' . 'Hành động đang chờ xử lý' . '</span>
					';
	}
	$__finalCompiled .= '
				</h3>
