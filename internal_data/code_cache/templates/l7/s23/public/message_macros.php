...
return array('macros' => array('user_info' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['user']['user_id'] AND $__templater->method($__vars['xf']['visitor'], 'canStartConversationWith', array($__vars['user'], ))) {
		...
		if ($__templater->func('property', array('xtr_conversation_button', ), false)) {
			$__finalCompiled .= '
						<a href="' . $__templater->func('link', array('conversations/add', '', array('to' => $__vars['user']['username'], ), ), true) . '" data-xf-click="' . 'overlay' . '" class="xm-icon button--link"><i class="fas fa-envelope" aria-hidden="true" data-xf-init="tooltip" title="' . 'Bắt đầu một cuộc trò chuyện' . '"></i></a>
					';
		}
		$__finalCompiled .= '
				';
	...
	$__finalCompiled .= '
		...
		'tag' => 'div',
		'class' => 'message-userBanner xgtUserBanner-mesaj',
		'itemprop' => 'jobTitle',
	))) . '
		</div>
		';
	if ($__vars['user']['user_id']) {
		...
		$__compilerTemp1 .= '
					';
		if ($__vars['extras']['resource_count'] AND $__vars['user']['xfrm_resource_count']) {
			$__compilerTemp1 .= '
						<dl class="pairs pairs--justified">
							<dt>' . 'Resources' . '</dt>
							<dd>
