...
return array('macros' => array('reaction_snippet' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['post']['user_id'] AND ($__vars['post']['user_id'] == $__vars['xf']['visitor']['user_id'])) {
		$__finalCompiled .= '
			' . '' . $__templater->func('username_link', array($__vars['reactionUser'], false, array('defaultname' => $__vars['fallbackName'], ), ), true) . ' đã cho bạn ' . $__templater->filter($__templater->func('alert_reaction', array($__vars['reactionId'], 'medium', ), false), array(array('preescaped', array()),), true) . ' trong bình luận của bạn trong bài viết: <a ' . (('href="' . $__templater->func('link', array('posts', $__vars['post'], ), true)) . '"') . '>' . ((((('<a href="' . $__templater->func('link', array('threads', $__vars['post']['Thread'], ), true)) . '">') . $__templater->func('prefix', array('thread', $__vars['post']['Thread'], ), true)) . $__templater->escape($__vars['post']['Thread']['title'])) . '</a>') . '</a>' . '
		';
	} else {
		$__finalCompiled .= '
