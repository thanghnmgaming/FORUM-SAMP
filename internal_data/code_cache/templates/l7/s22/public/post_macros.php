...
';
	...
	if ($__vars['post']['last_edit_date']) {
		...
		if ($__vars['post']['user_id'] == $__vars['post']['last_edit_user_id']) {
			...
			$__finalCompiled .= '
									' . 'Sửa lần cuối bởi điều hành viên' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['post']['last_edit_date'], array(
			))) . '
								';
		}
		$__finalCompiled .= '
	...
	$__compilerTemp4 .= '
											...
											';
	if ($__templater->method($__vars['xf']['visitor'], 'hasPermission', array('border', 'view', )) AND ($__vars['post']['attach_count'] > 0)) {
		$__compilerTemp4 .= '
    <a href="' . $__templater->func('link', array('posts/border', $__vars['post'], ), true) . '" class="actionBar-action actionBar-action--ip actionBar-action--menuItem">' . 'Border' . '</a>
';
	}
	$__compilerTemp4 .= '
';
