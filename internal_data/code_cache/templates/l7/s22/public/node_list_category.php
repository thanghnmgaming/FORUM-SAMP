<?php
// FROM HASH: cbe4ae69efbc7a1780276d54422b3ddd
...
'depth2' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="node node--id' . $__templater->escape($__vars['node']['node_id']) . ' node--depth' . $__templater->escape($__vars['depth']) . ' node--category ' . ($__vars['extras']['hasNew'] ? 'node--unread' : 'node--read') . '">
		<div class="node-body">
			';
	if ($__templater->method($__vars['node'], 'isCustomNode', array()) OR $__templater->method($__vars['node'], 'isCustomUnreadNode', array())) {
		$__finalCompiled .= '
	' . $__templater->callMacro('node_list_ext', 'node-icon', array(
			'node' => $__vars['node'],
...
'depthN' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__templater->method($__vars['node'], 'isCustomNode', array()) OR $__templater->method($__vars['node'], 'isCustomUnreadNode', array())) {
		$__finalCompiled .= '
	' . $__templater->callMacro('node_list_ext', 'node-icon-submenu', array(
			'node' => $__vars['node'],
			'extras' => $__vars['extras'],
			'subNodeLinkClass' => 'subNodeLink--category',
			'link' => $__templater->func('link', array('categories', $__vars['node'], ), false),
		), $__vars) . '
';
	} else {
		$__finalCompiled .= '
